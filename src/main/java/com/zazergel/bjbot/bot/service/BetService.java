...
public class BetService {
    ...
    public BotApiMethod<?> chooseStartBet(CallbackQuery callbackQuery) {
        ...
        checkUserBalance(chatId, callbackQuery);
    ...
    private void checkUserBalance(long chatId, CallbackQuery callbackQuery) {
        long userBalance = userRepo.findById(chatId).orElseThrow().getScore();
        Integer callbackQueryDate = callbackQuery.getMessage().getDate();
        LocalDateTime messageDateTime = LocalDateTime.ofInstant(
                Instant.ofEpochSecond(callbackQueryDate),
                ZoneId.systemDefault());
        long daysBetween = ChronoUnit.DAYS.between(messageDateTime, LocalDateTime.now());
        if (daysBetween >= 1) {
